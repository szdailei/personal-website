...
function startReactCompenent(ctx, text) {
  debug(DEBUG_NAME)(kuler(`@requires  待解析组件的文本：${text}`, BLUE));
  ...
  if (node.foundClosingTag) {
    finishReactComponent(ctx);
    debug(DEBUG_NAME)('@requires  在marked的同一个token里面包含closing tag');
    debug(DEBUG_NAME)('@ensure  结束组件的解析');
  } else {
    debug(DEBUG_NAME)('@requires  在marked的同一个token里面不包含closing tag');
